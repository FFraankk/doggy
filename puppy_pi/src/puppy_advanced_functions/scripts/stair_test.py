...
def move():
    ...
    while True:
        ...
        while(puppyStatus == PuppyStatus.LOOKING_FOR) :
            if target_centre_point != None and target_centre_point[1] >300:
                puppyStatus = PuppyStatus.FOUND_TARGET
                rospy.sleep(2.6) # 继续往前走一点
        ...
        while(puppyStatus == PuppyStatus.FOUND_TARGET) :
            if time.time() - up_stairs_time <=50:
                runActionGroup_srv('up_stairs_3.5cm0.d6ac',True)
            if time.time() - up_stairs_time >53:
                runActionGroup_srv('up_stairs_3.5cm1.d6ac',True)
            if time.time() - up_stairs_time >60:
...
def boxes_callback(BoundingBoxes):
    global __isRunning, puppyStatus, target_centre_point
    if __isRunning:
        if len(BoundingBoxes.bounding_boxes)==2:
            # if BoundingBoxes.bounding_boxes[1].probability>0.65:
                centerX=BoundingBoxes.bounding_boxes[1].xmax
                centerY=BoundingBoxes.bounding_boxes[1].ymax
                if puppyStatus != PuppyStatus.FOUND_TARGET:
                    target_centre_point = [centerX, centerY]
        rospy.sleep(0.001)
...
if __name__ == '__main__':
    ...
    BoundingBoxes_sub = rospy.Subscriber('/yolov8/BoundingBoxes', BoundingBoxes, boxes_callback)
